...
// MODIFIED: Add color for certain variable
Window_MainMenuVariable.prototype.drawVariableData = function (i, x, y) {
  ...
  const origin = $gameVariables.value(varId);
  var value = Yanfly.Util.toGroup(origin);
  var width = this.contents.width - this.textPadding() * 2;
  let color = '\\C[0]';
  if ([VARS.DEBT, VARS.POINT].includes(varId) && origin > 0) color = '\\C[10]';
  else if ([VARS.SECURITY, VARS.DISCIPLINE, VARS.RANK].includes(varId) && origin < 1)
    color = '\\C[10]';
  else if ([VARS.SECURITY, VARS.DISCIPLINE].includes(varId) && origin < 51) color = '\\C[21]';
  else if (varId === VARS.RANK && origin < 5) color = '\\C[21]';
  this.drawText(color + value, x, y, width, 'right');
...
Scene_Menu.prototype.getVariableWindowHeight = function () {
  if (Yanfly.Param.MMVarWinHeight.toUpperCase() === 'AUTO') {
    return Graphics.boxHeight - this._commandWindow.height - this._goldWindow.height;
  } else {
