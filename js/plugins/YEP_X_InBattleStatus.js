...
//=============================================================================
/*:
 * @plugindesc 【2023-4-6 QJ修改】 v1.01 (Requires YEP_BattleEngineCore.js) Adds a 'Status'
 ...
 * @default 暴击率
 *
 * @param helpLineNumber
 * @parent ---General---
 * @desc 帮助窗口最大行数，不决定帮助窗口实际的高度
 * @default 6
 *
 * @help
 *
 * 【2023-4-22 QJ修改】
 * 1.修改UI。
 * 2.新增脚本指令，让界面可以在地图界面打开。
 * 3.删除游戏内的状态选项，插件指令ShowInBattleStatus和HideInBattleStatus无效化。
 * 4.增加插件参数“暴击率的名字”。
 * 5.openStateDetailsPanel(actorId)
 * 6.进入界面
 *SceneManager.push(Scene_Status_NJYQ)
 *
 *
 *
 * ============================================================================
...
if (Imported.YEP_BattleEngineCore) {
  ...
  Yanfly.Param.IBScriName = String(Yanfly.Parameters['criName']);
  ...
  Yanfly.Param.IBShelpLineNumber = String(Yanfly.Parameters['helpLineNumber']);
  ...
  Game_System.prototype.getRealActorToShowIBS = function () {
    return this._realActorToShowIBS || 0;
  };

  Game_System.prototype.setRealActorToShowIBS = function (actorId) {
    this._realActorToShowIBS = actorId > 0 ? actorId : 0;
  };

  window.openStateDetailsPanel = function (actorId) {
    $gameSystem.setRealActorToShowIBS(actorId);
  };

  Imported.YEP_X_InBattleStatus = false;

  //=============================================================================
  // Game_Interpreter
  //=============================================================================

  Yanfly.IBS.Game_Interpreter_pluginCommand = Game_Interpreter.prototype.pluginCommand;
  ...
  Yanfly.IBS.Window_Base_convertEscapeCharacters = Window_Base.prototype.convertEscapeCharacters;
  ...
  if (Imported.YEP_X_MessageMacros1) {
    Yanfly.IBS.Window_Base_convertMacroText = Window_Base.prototype.convertMacroText;
  ...
  //=============================================================================
  /*QJ删除
  Yanfly.IBS.Window_PartyCommand_makeCommandList = Window_PartyCommand.prototype.makeCommandList;
  ...
  //=============================================================================
  // Window_InBattleStatusIBS
  //=============================================================================

  function Window_InBattleStatusIBS() {
    this.initialize.apply(this, arguments);
  }

  Window_InBattleStatusIBS.prototype = Object.create(Window_Base.prototype);
  Window_InBattleStatusIBS.prototype.constructor = Window_InBattleStatusIBS;

  Window_InBattleStatusIBS.prototype.initialize = function () {
  ...
  Window_InBattleStatusIBS.prototype.setBattler = function (battler) {
    this._battler = battler;
    this.refresh();
  };

  Window_InBattleStatusIBS.prototype.drawActorName = function () {};
  Window_InBattleStatusIBS.prototype.drawActorLevel = function () {};
  Window_InBattleStatusIBS.prototype.drawActorIcons = function () {};

  Window_InBattleStatusIBS.prototype.refresh = function () {
    this.contents.clear();
    ...
    if (h >= this.lineHeight() * 6) {
      ...
      this.drawParamExtra(x, y, w, this.lineHeight());
    } else {
      w = Math.floor(w / 2);
      x2 = x;
      for (var i = 2; i < 4; ++i) {
  ...
  Window_InBattleStatusIBS.prototype.drawParam = function (paramId, dx, dy, dw, dh) {
  ...
  Window_InBattleStatusIBS.prototype.drawParamExtra = function (dx, dy, dw, dh) {
    this.drawDarkRect(dx, dy, dw, dh);
    dx += Window_Base._iconWidth + 4;
    dw -= Window_Base._iconWidth + 4 + this.textPadding() + 2;
    this.changeTextColor(this.systemColor());
    this.drawText(Yanfly.Param.IBScriName, dx, dy, dw);
    var value = this._battler.cri;
    this.changeTextColor(this.paramchangeTextColor(0));
    this.drawText((value * 100).toFixed(2) + '%', dx, dy, dw, 'right');
  };

  /*Window_InBattleStatusIBS.prototype.drawDarkRect = function (dx, dy, dw, dh) {
  ...
  Window_InBattleStatusIBS.prototype.drawDarkRect = function () {};

  //=============================================================================
  // Window_InBattleStateListIBS
  //=============================================================================

  function Window_InBattleStateListIBS() {
    this.initialize.apply(this, arguments);
  }

  Window_InBattleStateListIBS.prototype = Object.create(Window_Selectable.prototype);
  Window_InBattleStateListIBS.prototype.constructor = Window_InBattleStateListIBS;

  Window_InBattleStateListIBS.prototype.initialize = function (parentWindow) {
    this.preloadFaceImage();
  ...
  Window_InBattleStateListIBS.prototype.preloadFaceImage = function () {
  ...
  Window_InBattleStateListIBS.prototype.setStatusWindow = function (win) {
    this._statusWindow = win;
  };

  Window_InBattleStateListIBS.prototype.setBattler = function (battler) {
    this._battler = battler;
  ...
  Window_InBattleStateListIBS.prototype.maxItems = function () {
    return this._data ? this._data.length : 1;
  };

  Window_InBattleStateListIBS.prototype.item = function () {
    var index = this.index();
    return this._data && index >= 0 ? this._data[index] : null;
  };

  Window_InBattleStateListIBS.prototype.includes = function (item) {
  ...
  Window_InBattleStateListIBS.prototype.makeItemList = function () {
    this._data = [];
    if (this._battler) {
      ...
      for (var i = 0; i < 8; ++i) {
        if (this._battler.isBuffAffected(i) || this._battler.isDebuffAffected(i)) {
  ...
  Window_InBattleStateListIBS.prototype.drawItem = function (index) {
  ...
  Window_InBattleStateListIBS.prototype.updateHelp = function () {
    if (this.item() === null) {
      var text = Yanfly.Param.IBSHealthyHelp;
      this._helpWindow.setText(text);
    } else if (typeof this.item() === 'string' && this.item().match(/BUFF[ ](\d+)/i)) {
      var paramId = parseInt(RegExp.$1);
      var level = this._battler._buffs[paramId];
      if (level > 0) {
  ...
  Window_InBattleStateListIBS.prototype.refresh = function () {
    this.makeItemList();
    this.createContents();
    this.drawAllItems();
  };

  Window_InBattleStateListIBS.prototype.update = function () {
    if (this.updateActorToShowData()) return;
    Window_Selectable.prototype.update.call(this);
    if (this.active && this._battler) this.updateLeftRight();
    if (SceneManager._scene && SceneManager._scene.constructor.name !== 'Scene_Status_NJYQ') {
      if (TouchInput.isCancelled() || Input.isTriggered('ok') || Input.isTriggered('cancel')) {
        SoundManager.playCancel();
        openStateDetailsPanel(0);
      }
    }
  };

  Window_InBattleStateListIBS.prototype.updateActorToShowData = function () {
    let targetActor = $gameSystem.getRealActorToShowIBS();
    if (this._remActorId === targetActor) {
      return this._remActorId === 0;
    } else {
      this._remActorId = targetActor;
      if (targetActor === 0) {
        this.visible = false;
        this._helpWindow.visible = false;
        this._parentWindow.visible = false;
        return true;
      } else {
        this.visible = true;
        this._helpWindow.visible = true;
        this._parentWindow.visible = true;
        this.activate();
        this.setBattler($gameActors.actor(targetActor));
      }
    }
  };

  Window_InBattleStateListIBS.prototype.updateLeftRight = function () {
  ...
  if (Imported.YEP_X_BattleSysCTB) {
    Yanfly.IBS.Window_CTBIcon_isReduceOpacity = Window_CTBIcon.prototype.isReduceOpacity;
  ...
  Yanfly.IBS.Scene_Battle_createAllWindows = Scene_Battle.prototype.createAllWindows;
  ...
  Scene_Battle.prototype.createInBattleStatusWindows = function () {
    this._normalStatusWindowIBS = new Window_InBattleStatusIBS();
    this.addChild(this._normalStatusWindowIBS);

    this._normalHelpWindowIBS = new Window_Help(Yanfly.Param.IBShelpLineNumber);
    this._normalHelpWindowIBS.height = this._normalStatusWindowIBS.y;
    this.addChild(this._normalHelpWindowIBS);

    this._normalStateListWindowIBS = new Window_InBattleStateListIBS(this._normalStatusWindowIBS);
    this._normalStateListWindowIBS.setHelpWindow(this._normalHelpWindowIBS);
    this.addChild(this._normalStateListWindowIBS);
  };

  Yanfly.IBS.Scene_Battle_updateChildren = Scene_Battle.prototype.updateChildren;
  Scene_Battle.prototype.updateChildren = function () {
    if ($gameSystem.getRealActorToShowIBS() > 0) {
      this._normalStatusWindowIBS.update();
      this._normalHelpWindowIBS.update();
      this._normalStateListWindowIBS.update();
    } else {
      Yanfly.IBS.Scene_Battle_updateChildren.call(this);
    }
  };

  Yanfly.IBS.Scene_Map_createAllWindows = Scene_Map.prototype.createAllWindows;
  Scene_Map.prototype.createAllWindows = function () {
    Yanfly.IBS.Scene_Map_createAllWindows.call(this);
    this.createInBattleStatusWindows();
  };

  Scene_Map.prototype.createInBattleStatusWindows =
    Scene_Battle.prototype.createInBattleStatusWindows;

  Yanfly.IBS.Game_Message_isBusy = Game_Message.prototype.isBusy;
  Game_Message.prototype.isBusy = function () {
    if (SceneManager._scene && SceneManager._scene.constructor.name === 'Scene_Map') {
      return (
        Yanfly.IBS.Game_Message_isBusy.apply(this, arguments) ||
        $gameSystem.getRealActorToShowIBS() > 0
      );
    } else {
      return Yanfly.IBS.Game_Message_isBusy.apply(this, arguments);
    }
  };
  ...
  function Scene_Status_NJYQ() {
    this.initialize.apply(this, arguments);
  }
  window.Scene_Status_NJYQ = Scene_Status_NJYQ;
  Scene_Status_NJYQ.prototype = Object.create(Scene_MenuBase.prototype);
  Scene_Status_NJYQ.prototype.constructor = Scene_Status_NJYQ;

  Scene_Status_NJYQ.prototype.initialize = function () {
    Scene_MenuBase.prototype.initialize.call(this);
  };

  Scene_Status_NJYQ.prototype.create = function () {
    Scene_MenuBase.prototype.create.call(this);
    this.createInBattleStatusWindows();
    openStateDetailsPanel($gameParty.menuActor().actorId());
  };

  Scene_Status_NJYQ.prototype.createInBattleStatusWindows =
    Scene_Battle.prototype.createInBattleStatusWindows;

  Scene_Status_NJYQ.prototype.update = function () {
  ...
  Yanfly.IBS.Scene_Battle_isAnyInputWindowActive = Scene_Battle.prototype.isAnyInputWindowActive;
