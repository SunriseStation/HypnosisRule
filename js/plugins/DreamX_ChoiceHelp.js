...
(function () {
  ...
  var paramNumRows = eval(DreamX.Parameters['Number Of Rows'] || false);
  var paramCloseHelpOnEmpty = eval(DreamX.Parameters['Hide On Empty'] || 2);
  var sw = Number(DreamX.Parameters['sw'])
  ...
  Window_ChoiceList.prototype.updateHelp = function () {
    var help = $gameMessage._choiceHelps && $gameMessage._choiceHelps[this.index()];
    if (help) {
		   help = help.replace(/\\n/g, '\n');
	       help = help.replace(/\\/g, '\x1b');
           help = help.replace(/\x1b\x1b/g, '\\');
		   help = $gameSwitches._data[sw] ? '<WordWrap>' + help : help;
		   this._helpWindow.setText(help ? help : '');
    ...
    var message = $gameMessage._choiceMessages && $gameMessage._choiceMessages[this.index()];
    var face = $gameMessage._choiceFaces && $gameMessage._choiceFaces[this.index()];
    ...
    if (this.hasChoiceMessage === true) {
      ...
      if (message) {
        ...
        messageWindow._textState.height = messageWindow.calcTextHeight(
          messageWindow._textState,
          false,
        );
