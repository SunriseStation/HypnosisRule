...
Shora.warn = function (err) {
  const message = new PIXI.Text('Shora Lighting Plugin: ' + err, {
    fontFamily: 'Arial',
    fontSize: 12,
    fill: 0xffffff,
    align: 'left',
  });
  message.y += Shora.MessageY;
  Shora.MessageY += message.height;
  ...
  Game_Interpreter.prototype.pluginCommand = function (command, args) {
    ...
    if (command) {
      ...
      if (command === 'ambient') {
        ...
        } else if (command === 'shadow') $gameLighting.setShadow(id, args[1]);
      }
    }
  };
} else {
...
Shora.CallCommand = function (settings, command) {
  if (!command || command.length <= 2 || command[0] !== '[' || command[command.length - 1] !== ']')
    return;
  // [<name>, <param1>, <value1>, ..]
...
if (JSON.parse(Shora.Lighting.PARAMETERS.helper).disableEngineShadow === 'true') {
  ...
  if (Shora.EngineVersion === 'MV') ShaderTilemap.prototype._addShadow = function () {};
}
...
(_ => {
  ...
  _.extractSaveContents = function (contents) {
    ...
    if (!contents.lighting) $gameLighting = new GameLighting();
  };
})(DataManager);
...
(_ => {
  ...
  _.destroy = function (options) {
    if ($shoraLayer.lighting) $shoraLayer.removeScene(this);
    destroy.call(this, options);
  };
  const createUpperLayer = _.createUpperLayer;
  _.createUpperLayer = function () {
    if (!$gameLighting._disabled) $shoraLayer.loadScene(this);
  ...
  _.update = function () {
    update.call(this);
    if (!$gameLighting._disabled) $shoraLayer.update();
  };
  ...
  _.scanNoteTags = function (lines) {
    for (command of lines) {
      ...
      switch (command[1].toLowerCase()) {
        case 'ambient':
          if ($shoraLayer.lighting) $gameLighting.setMapAmbient(command[2]);
          else $gameLighting.ambient = command[2].toHexValue();
          break;
        case 'shadowambient':
...
(_ => {
  ...
  _.scanLighting = function () {
    ...
    if ($gameParty.leader()) note = $gameParty.leader().actor().note.split('\n');
    for (let line of note) Shora.CallCommand(lightingParams, line);
    if (lightingParams.name) {
      ...
      for (const item of $gameParty.items()) {
        ...
        for (let line of note) Shora.CallCommand(lightingParams, line);
        if (lightingParams.name) return this.setLighting(lightingParams);
      }
      this.hasLight = false;
    }
  };
...
class Layer {
  ...
  load(name) {
    if (!this.baseTextureCache[name]) {
      if (name == undefined)
        throw new Error(
          "Please don't change default lighting reference and set it back to 'default'",
        );
      else throw new Error('Please add + ' + name + '.png light image to /img/lights/.');
    }
  ...
  addLighting(_settings) {
    ...
    if (name == '<-- CHANGE_THIS -->')
      return console.warn(
        'Please set the reference of light, aka it name when adding new custom light. Register progress canceled.',
      );
  ...
  updateIntensityFilter(spriteset, disable) {
    if (!disable && this._colorFilter.status == 'true') {
      ...
      if (Shora.EngineVersion == 'MV') spriteset._baseSprite.filters[0].brightness(1);
      else spriteset._baseSprite.filters[0].setBrightness(255);
    }
  }
  ...
  loadScene(spriteset) {
    ...
    if (!this.lighting) this.lighting = new LightingLayer();
    Shora.MessageY = 0;
    // this.updateIntensityFilter(this._spriteset);
    if (
      $gameMap.mapId() === this.mapId &&
      this._spriteset.type() == this._spritesetType &&
      this.lighting
    )
  ...
  update() {
    if ($gameMap.mapId() != this.mapId) (this.mapId = $gameMap.mapId()), ($gameMap._lighting = []);
...
class LightingLayer {
  constructor() {
    this.lights = [];
    this.softShadowFilters = [
      new KawaseBlurFilter($gameLighting.softShadowStr, $gameLighting.softShadowQlt),
    ];
  ...
  initialize() {
    ...
    for (const light of this.lights) if (light) this.removeLight(light.id);
    this.lights = [];
    // shadow
    $gameShadow.refresh();
  ...
  addLightingSprite() {
    for (const light of $gameMap._lighting) if (light) this.addLight(light);
  ...
  addStaticLight(options) {
    ...
    if (Shora.EngineVersion == 'MV') light.blendMode = PIXI.BLEND_MODES.NORMAL; // TODO
   ...
   */
  removeLight(id) {
    if (!this.lights[id]) return Shora.warn('cant remove light' + id);
...
class LightingSprite extends PIXI.Sprite {
  ...
  constructor(options) {
    ...
    this.shadow = new Shadow(
      ...
      this.worldY(),
      this.worldBound(),
      options.shadowambient,
      this.shadowOffsetX,
      this.shadowOffsetY,
    );
    if (this._shadow) this.shadow.render(this.texture);
  ...
  update() {
    if (!this.character) return $shoraLayer.lighting.removeLight(this.id);
    if (!this.status) return (this.renderable = false);
  ...
  needRecalculateShadow() {
    if (this.offset.updating()) return true;
  ...
  setPostion(options) {
    this.x =
      options.x != undefined
        ? $gameShadow.worldToScreenX(options.x)
        : this.character.screenX() + this.offset.x;
    this.y =
      options.y != undefined
        ? $gameShadow.worldToScreenY(options.y)
...
class Shadow {
  ...
  destroy() {
    this.polygon = this.bounds = this._parallelSegments = this.shadowAmbient = null;
  ...
  drawWall(index, oy, lowerWalls) {
    let [x, y] = this.polygon[index],
      last = index == 0 ? this.polygon.length - 1 : index - 1;
    ...
    for (let i = 0; i < lowerWalls.length; ++i) {
      ...
      if (y == ny && y == y1 && x >= x1 && x <= x2 && nx >= x1 && nx <= x2 && oy >= y1) {
        this.graphics.lineTo(nx, ny - tw * height).lineTo(x, y - tw * height);
      }
    }
  }
  ...
  containParallelSegment(y, u, v) {
    ...
    let lo = 0,
      hi = this._parallelSegments[y].length - 1,
      result = -1;
    while (lo <= hi) {
      let mid = (lo + hi) >> 1;
  ...
  draw(oy, lowerWalls) {
    ...
    if (this.polygon[0][1] == this.polygon[this.polygon.length - 1][1]) {
      if (!this._parallelSegments[this.polygon[0][1]])
        this._parallelSegments[this.polygon[0][1]] = [];
      this._parallelSegments[this.polygon[0][1]].push([
        this.polygon[0][0],
        this.polygon[this.polygon.length - 1][0],
      ]);
    }
    ...
    for (let y in this._parallelSegments) {
      for (let i in this._parallelSegments[y]) {
        if (this._parallelSegments[y][i][0] > this._parallelSegments[y][i][1])
          [this._parallelSegments[y][i][0], this._parallelSegments[y][i][1]] = [
            this._parallelSegments[y][i][1],
            this._parallelSegments[y][i][0],
          ];
      }
  ...
  const compute = (position, segments) => {
    ...
    for (let i = 0; i < sorted.length; ) {
      ...
      if (extend) {
        ...
        polygon.push(
          intersectLines(bounded[old_segment][0], bounded[old_segment][1], position, vertex),
        );
...
class ScaleAnimation extends Shora.Animation {
  ...
  update() {
    if (this.tick <= this.time) {
      this._ref.radius =
        this.s0 + Shora.Animation.transition[this.type](this.tick / this.time) * this.delta;
...
class AngleAnimation extends Shora.Animation {
  ...
  update() {
    ...
    if (this.tick <= this.time) {
      this._ref.angle = this._sprite.rotation =
...
class TintAnimation extends Shora.Animation {
  ...
  update() {
    if (this.tick <= this.len) {
      ...
      this._ref.tint = this._sprite.tint = Shora.ColorManager.transition(
        p,
        this.ocolor,
        this.dcolor,
      );
...
class OffsetAnimation {
  constructor(offset) {
    ...
    this.oy = offset.y;
    this.tick_x = 2;
    this.time_x = this.delta_x = 1;
    this.tick_y = 2;
    this.time_y = this.delta_y = 1;
  ...
  update() {
    if (this.tick_x <= this.time_x) {
      this.offset.x =
        this.ox + Shora.Animation.transition[this.type_x](this.tick_x / this.time_x) * this.delta_x;
      this.tick_x++;
    }
    if (this.tick_y <= this.time_y) {
      this.offset.y =
        this.oy + Shora.Animation.transition[this.type_y](this.tick_y / this.time_y) * this.delta_y;
      this.tick_y++;
    }
  }
...
GameLighting.prototype.remove = function (id) {
  if ($gameMap._lighting[id]) $shoraLayer.lighting.removeLight(id);
};
...
GameLighting.prototype.setStatus = function (id, status) {
  ...
  $gameMap._lighting[id].status = $shoraLayer.lighting.lights[id].status =
...
GameLighting.prototype.setPluginState = function (status) {
  if (status && this._disabled) this.enable();
  if (!status && !this._disabled) this.disable();
};

GameLighting.prototype.enable = function () {
  if (!this._disabled) return;
  this._disabled = false;
  if (SceneManager._scene._spriteset) $shoraLayer.loadScene(SceneManager._scene._spriteset);
};

GameLighting.prototype.disable = function () {
  if (this._disabled) return;
  this._disabled = true;
  if (SceneManager._scene._spriteset) $shoraLayer.removeScene(SceneManager._scene._spriteset);
};
...
class ShadowCaster {
  constructor(p, height) {
    this.height = height;
    this.segments = [];
    for (let i = 0; i < p.length - 1; ++i) this.segments.push([p[i], p[i + 1]]);
...
class GameShadow {
  ...
  scanMapCaster() {
    this.map = new Array($gameMap.height()).fill(0).map(() => new Array($gameMap.width()).fill(0));
    ...
    for (var i = 0; i < $gameMap.height(); ++i) {
      ...
      for (var j = 0; j < $gameMap.width(); ++j) {
        ...
        if (
          (regionStart <= $gameMap.regionId(j, i) && $gameMap.regionId(j, i) <= regionEnd) ||
          $gameMap.regionId(j, i) == topRegionId
        ) {
        ...
        if ($gameMap.regionId(j, i) == ignoreShadowsId) this.ignoreShadows.push([j * tw, i * tw]);
      }
    }
