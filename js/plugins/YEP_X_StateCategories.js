...
if (Imported.YEP_BuffsStatesCore) {
	...
	Yanfly.StC.Game_BattlerBase_recoverAll =
	...
	Game_BattlerBase.prototype.clearStates = function () {
		if (this.isCustomClearStates()) {
			var states = this._states && JsonEx.makeDeepCopy(this._states);
			var turns = this._stateTurns && JsonEx.makeDeepCopy(this._stateTurns);
			var details =
				this._stateDetails && JsonEx.makeDeepCopy(this._stateDetails);
		}
		Yanfly.StC.Game_BattlerBase_clearStates.call(this);
		if (this.isCustomClearStates())
			this.retainCustomClearStates(states, turns, details);
	};
		...
		for (var i = 0; i < length; ++i) {
			...
			if ($gameTemp._deathStateClear) {
				if (state.category.contains('BYPASS DEATH REMOVAL')) {
					...
					this._stateDetails[id] = details[id];
					removed = true;
				}
			}
	...
	Yanfly.StC.Game_Battler_isStateAddable =
