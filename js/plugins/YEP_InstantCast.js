...
BattleManager.endActorInstantCast = function () {
  ...
  this._phase = 'input';
  //! MODIFIED: For some reason, skills not marking Instant will be running into this process, this is a temp fix for not generating error
  if (user.canMove() && user.canInput()) {
    user.endInstantCast && user.endInstantCast();
  } else {
    user.makeActions();
...
if (Imported.YEP_BattleEngineCore) {
  ...
  Yanfly.Instant.BattleManager_setPreForceActionSettings = BattleManager.setPreForceActionSettings;
  BattleManager.setPreForceActionSettings = function () {
    var settings = Yanfly.Instant.BattleManager_setPreForceActionSettings.call(this);
...
Yanfly.Instant.Scene_Battle_selectNextCommand = Scene_Battle.prototype.selectNextCommand;
...
Yanfly.Instant.Window_Base_drawItemName = Window_Base.prototype.drawItemName;
